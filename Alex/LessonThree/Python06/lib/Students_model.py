...
class student_Model:
    ...
    def __init__(self,name,age,account,password,lobj):
        ...
        for lesson in self.Lobj:
            self.__schedule[lesson] = 0    #课程进度率字典
            self.__num[lesson] = 0         #课程休业次数

    def Classbegins(self,lesson_name):      #上课
        for obj in self.Lobj:
            if obj.Lname == lesson_name :
                print('{0}课程进行第{1}次修业...'.format(obj.Lname, str(self.__num[obj] + 1)))
                if self.success_Radio():
                    tag = obj.classBegins()
                    if tag:
                        self.__schedule[obj] += 20
                        self.__num[obj] += 1
                        print ('你感觉非常好！')
                        print ('{0}:{1}课程学习成功！完成率提高20%，达成率变为{2}%'.format(self.Name,obj.Lname,str(self.__schedule[obj])))
                    else:
                        self.__num[obj] += 1
                        print ('你突然有一种骂娘的冲动...')
                        print ('{0}:{1}课程学习失败，你对课程的理解没有丝毫提高，达成率为{2}%'.format(self.Name,obj.Lname,str(self.__schedule[obj])))
                else:
                    tag = obj.classBegins()
                    if tag:
                        self.__schedule[obj] += 10
                        self.__num[obj] += 1
                        print('{0}:{1}课程学习欠佳！完成率提高10%，达成率变为{2}%'.format(self.Name,obj.Lname, str(self.__schedule[obj])))
                    else:
                        self.__num[obj] += 1
                        print('你突然萌生了轻生的念头...')
                        print('{0}:{1}课程学习失败，你对课程的理解没有丝毫提高，达成率为{2}%'.format(self.Name, obj.Lname,
                                                                            str(self.__schedule[obj])))
            else:
                pass
